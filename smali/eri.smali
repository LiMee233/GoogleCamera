.class final Leri;
.super Ljava/lang/Object;

# interfaces
.implements Lgjs;


# instance fields
.field public final a:Lpic;

.field public final b:Ljava/lang/Runnable;

.field public final c:Lpic;

.field public final d:I

.field public e:Lljg;

.field public f:Z

.field final synthetic g:Lerj;


# direct methods
.method public constructor <init>(Lerj;ILjava/lang/Runnable;)V
    .locals 0

    iput-object p1, p0, Leri;->g:Lerj;

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    invoke-static {}, Lpic;->f()Lpic;

    move-result-object p1

    iput-object p1, p0, Leri;->a:Lpic;

    invoke-static {}, Lpic;->f()Lpic;

    move-result-object p1

    iput-object p1, p0, Leri;->c:Lpic;

    const/4 p1, 0x0

    iput-boolean p1, p0, Leri;->f:Z

    iput p2, p0, Leri;->d:I

    iput-object p3, p0, Leri;->b:Ljava/lang/Runnable;

    return-void
.end method


# virtual methods
.method public final declared-synchronized a()Lpho;
    .locals 3

    monitor-enter p0

    :try_start_0
    iget-object v0, p0, Leri;->g:Lerj;

    iget-object v0, v0, Lerj;->d:Ljava/util/concurrent/Executor;

    new-instance v1, Lerf;

    const/4 v2, 0x0

    invoke-direct {v1, p0, v2}, Lerf;-><init>(Leri;I)V

    invoke-interface {v0, v1}, Ljava/util/concurrent/Executor;->execute(Ljava/lang/Runnable;)V

    iget-object v0, p0, Leri;->a:Lpic;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return-object v0

    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method public final b()Lpho;
    .locals 1

    const/4 v0, 0x0

    invoke-static {v0}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v0

    invoke-static {v0}, Loxc;->B(Ljava/lang/Object;)Lpho;

    move-result-object v0

    return-object v0
.end method

.method public final declared-synchronized c(Z)V
    .locals 2

    monitor-enter p0

    :try_start_0
    invoke-virtual {p0}, Leri;->e()V

    iget-object v0, p0, Leri;->a:Lpic;

    invoke-static {p1}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v1

    invoke-virtual {v0, v1}, Lpic;->o(Ljava/lang/Object;)Z

    if-nez p1, :cond_0

    const/4 p1, 0x1

    iput-boolean p1, p0, Leri;->f:Z

    iget-object v0, p0, Leri;->c:Lpic;

    invoke-virtual {v0, p1}, Lpic;->cancel(Z)Z
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    :cond_0
    monitor-exit p0

    return-void

    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public final declared-synchronized d(Ljava/lang/Runnable;Ljava/lang/Runnable;)V
    .locals 2

    monitor-enter p0

    :try_start_0
    iget-object v0, p0, Leri;->c:Lpic;

    invoke-virtual {v0}, Lpic;->isCancelled()Z

    move-result v0

    if-nez v0, :cond_0

    iget-boolean v0, p0, Leri;->f:Z

    if-nez v0, :cond_0

    iget-object v0, p0, Leri;->c:Lpic;

    new-instance v1, Lerh;

    invoke-direct {v1, p0, p1, p2}, Lerh;-><init>(Leri;Ljava/lang/Runnable;Ljava/lang/Runnable;)V

    sget-object p1, Lpgm;->a:Lpgm;

    invoke-static {v0, v1, p1}, Loxc;->L(Lpho;Lphc;Ljava/util/concurrent/Executor;)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return-void

    :cond_0
    :try_start_1
    sget-object p1, Lerj;->a:Loue;

    invoke-virtual {p1}, Lotz;->c()Louv;

    move-result-object p1

    sget-object p2, Lovg;->a:Louy;

    const-string v0, "LasagnaProcQueue"

    invoke-interface {p1, p2, v0}, Louv;->g(Louy;Ljava/lang/Object;)Louv;

    move-result-object p1

    check-cast p1, Loub;

    const/16 p2, 0x592

    invoke-interface {p1, p2}, Loub;->G(I)Louv;

    move-result-object p1

    check-cast p1, Loub;

    const-string p2, "Cannot execute, already cancelled %s"

    iget v0, p0, Leri;->d:I

    invoke-interface {p1, p2, v0}, Loub;->p(Ljava/lang/String;I)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    monitor-exit p0

    return-void

    :catchall_0
    move-exception p1

    monitor-exit p0

    throw p1
.end method

.method public final e()V
    .locals 1

    iget-object v0, p0, Leri;->e:Lljg;

    if-eqz v0, :cond_0

    invoke-interface {v0}, Lljg;->a()V

    :cond_0
    return-void
.end method
